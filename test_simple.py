...
def parse_args():
    ...
    parser.add_argument('--model_name',
                        ...
                        choices=[
                            "mono_640x192", "stereo_640x192",
                            "mono+stereo_640x192", "mono_no_pt_640x192",
                            "stereo_no_pt_640x192",
                            "mono+stereo_no_pt_640x192", "mono_1024x320",
                            "stereo_1024x320", "mono+stereo_1024x320"
                        ])
...
def evaluate_single(image, model_name="mono+stereo_640x192"):
    """Function to predict for a single image or folder of images
    """
    # assert args.model_name is not None, \
    #     "You must specify the --model_name parameter; see README.md for an example"
    ...
    with torch.no_grad():
        ...
        colormapped_im = (mapper.to_rgba(disp_resized_np)[:, :, :3] *
                          255).astype(np.uint8)
        im = pil.fromarray(colormapped_im)
        return (scaled_disp.cpu().numpy(), im)
        # name_dest_im = os.path.join(output_directory,
        # "{}_disp.jpeg".format(output_name))
        # im.save(name_dest_im)
        #
